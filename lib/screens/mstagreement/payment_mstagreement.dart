...
class PaymentMstAgreement extends StatefulWidget {
  final List data;

  const PaymentMstAgreement({
    super.key,
    required this.data,
  });

  @override
  State<PaymentMstAgreement> createState() => _PaymentMstAgreementState();
}

class _PaymentMstAgreementState extends State<PaymentMstAgreement> {
  bool loading = false;
  ...
  Future<void> getPaymentMSTAgreement() async {
    try {
      setState(() {
        loading = true;
      });
      ...
      final response = await http.post(Uri.parse(url_GetMstAgreementPayment),
          headers: <String, String>{
            ...
            'SOAPAction': 'http://tempuri.org/GetMstAgreementPayment',
          ...
          await showDialog(
              ...
              builder: (BuildContext context) {
                return LogError(
                  ...
                  fail: 'Error Get Master Agreement Payment',
  ...
  Future<void> getAgreementAttachment(index) async {
    try {
      ...
      final String soapEnvelope = '<?xml version="1.0" encoding="utf-8"?>' +
          '<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">' +
          '<soap:Body>' +
          '<GetMstAgreementPayAttachment xmlns="http://tempuri.org/">' +
          '<NoAgreement>${widget.data.last['noAgreement']}</NoAgreement>' +
          '<Item>${payment[index]['item']}</Item>' +
          '<JenisAttachment>BUKTI TERIMA PEMBAYARAN</JenisAttachment>' +
          '<server>${widget.data.last['server']}</server>' +
          '</GetMstAgreementPayAttachment>' +
          '</soap:Body>' +
          '</soap:Envelope>';

      final response = await http.post(Uri.parse(url_GetMstAgreementPayAttachment),
          headers: <String, String>{
            "Access-Control-Allow-Origin": "*",
            'SOAPAction': 'http://tempuri.org/GetMstAgreementPayAttachment',
            'Access-Control-Allow-Credentials': 'true',
            'Content-type': 'text/xml; charset=utf-8'
          },
          body: soapEnvelope);
      ...
      if (response.statusCode == 200) {
        ...
        for (final listResult in listResultAll) {
          ...
          if (statusData == "GAGAL") {
            ...
            final pdfFile = listResult.findElements('FileUpload').isEmpty
                ? 'No Data'
                ...
                SizedBox(
                  ...
                  child: PaginatedDataTable2(
                      ...
                      source: TableData(
                          ...
                          rowsPerPage: rowPerPages,
                          status: widget.data.last['status'],
                          action: (index) async {
                            setState(() {
                              att.clear();
                              loading = true;
                            ...
                            if (mounted) {
                              // Menggunakan addPostFrameCallback untuk menjalankan operasi setelah widget dibangun
                              WidgetsBinding.instance
                                  .addPostFrameCallback((_) async {
                                if (att.isNotEmpty) {
                                  String ext = att.last['ext'].toString();

                                  if (ext.contains('pdf')) {
                                    Navigator.of(context)
                                        .push(MaterialPageRoute(
                                      builder: (BuildContext context) {
                                        return DisplayPDF(pdf: att);
                                      },
                                    ));
  ...
  DataRow? getRow(int index) {
    ...
    return DataRow2(
      color: WidgetStateProperty.all<Color?>(
        index.isOdd ? Colors.grey.withValues(alpha: 0.3) : null,
      ),
      cells: <DataCell>[
        DataCell(
          Center(
